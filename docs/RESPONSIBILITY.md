...
| SV-270558 | Restrict functions/ports/protocols/services per PPSM CAL (CM-7 b / CM-7(1)(b)) | DISA check is listener/Oracle-Net-configuration: inspect the listener config files (`SQLNET.ora`/`LISTENER.ora`/`TNSNAMES.ora`) under `$ORACLE_HOME/network/admin` for unauthorized ports/protocols; the fix edits them and runs `lsnrctl reload`. Both need OS/listener access the tenant lacks on managed RDS — the listener and config files are AWS-managed and unreachable, and the inherited baseline is a manual-review skip. On brokered Cloud.gov RDS the posture is platform-hardened two layers deep: the broker provisions the SSL option group so approved transport is TCPS/TLS on **port 2484**, and Cloud.gov platform **security groups restrict inbound to 2484 only, denying cleartext 1521** (terraform-provision#2361). Inherited from the platform. `set_by: aws_inherited` / `verified_by: not_applicable_rds`. |
| SV-270566 | Enforce authorized access to the PKI private key (IA-5(2)(a)(1), high) | DISA check reviews access controls on the DBMS private key — stored in an Oracle Wallet referenced by `sqlnet.ora` `WALLET_LOCATION` on the DB host — and the fix hardens access to that wallet/key. On managed AWS RDS the server-side TLS credentials (RDS Oracle wallet, X.509 certificate, and **private key**) are provisioned and managed entirely by AWS as part of the RDS SSL/TLS integration (broker SSL option group); the wallet and key live on the AWS-managed DB host filesystem the tenant cannot reach (no OS access to `sqlnet.ora`/the wallet; the broker DB user cannot read or relocate it). Access control over the database PKI private key is an AWS platform responsibility. Client-side wallet/key protection on the tenant's own systems is a separate customer operational concern outside this control's DB scope. Inherited from the platform. `set_by: aws_inherited` / `verified_by: not_applicable_rds`. |
...
| SV-270565 | Transmit only encrypted representations of passwords (IA-5(1)(c)) | Procedural DISA check (inspect `sqlnet.ora` for TLS/wallet entries; `sqlnet.ora` is AWS-managed/unreachable on RDS, no tenant SQL predicate). Satisfied by the **aws-broker TLS implementation**: the broker provisions the RDS SSL option group (TLS 1.2, FIPS cipher, TCPS on **port 2484**), requires TLS with host/server-DN verification (`SSL_SERVER_DN_MATCH`), and provides the trusted CA certificate(s) to the client, encrypting password traffic in transit. Configured in the AWS-managed `sqlnet.ora`/option group, not tenant SQL — a documented broker/platform disposition (see `control-layers.yml` `SV-270565` + the "during transmission" SSL-option-group entry). |
