...
If fields do not contain enabled constraints where required, this is a finding.'
  # MODIFIED FROM UPSTREAM (overlay #6 / mitre#1): upstream used `%q(...)` whose
  # `)` delimiter collided with unbalanced parens in the SQL examples below,
  # making the whole profile fail to load. Switched to `%q{...}` (block has no
  # braces). Reported upstream.
