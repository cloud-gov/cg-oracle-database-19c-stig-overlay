...
SCOPE = BOTH;"
  ...
  # OVERLAY-AUTHORED (overlay #6): upstream shipped this as an empty stub. No 12c
  # analogue exists (max_idle_time is a 19c-era parameter), so this check is
  # written from the DISA V1R5 check text: MAX_IDLE_TIME must be set (non-zero),
  # defaulting to a 15-minute expectation when no org value is documented. The
  # `max_idle_time` input lets an ISSO set the documented value.
  ...
  expected = input('max_idle_time', value: 15)
  values = sql.query("SELECT gp.value AS value FROM sys.gv_$parameter gp WHERE gp.name = 'max_idle_time';").column('value')
  if values.empty?
    describe 'max_idle_time parameter (no rows returned)' do
      subject { values }
      it { should_not be_empty }
    end
  else
    values.each do |v|
      describe "max_idle_time (minutes); 0/unset is a finding, must meet documented expectation (#{expected})" do
        subject { v.to_i }
        it { should be > 0 }
