...
| SV-270580 | Check validity of data inputs (SI-10) | Procedural source-code/schema review of the customer-owned application schema (field definitions, constraints, triggers). No portable pass/fail SQL predicate (a generic query cannot decide required constraints across arbitrary customer tables); customer application-design responsibility. See `controls/overlay.rb`. |
| SV-270581 | Reserve dynamic code execution for situations that require it (SI-10) | Procedural source-code review of customer-owned stored procedures/functions/triggers and application code. No portable pass/fail SQL predicate; customer secure-coding responsibility. See `controls/overlay.rb`. |
| SV-270582 | SQL-injection protection for dynamic code execution (SI-10) | Procedural source-code review of customer-owned dynamic code; no customer code runs at platform provision and there is no portable SQL predicate. Customer secure-coding responsibility (sibling to SV-270581). See `controls/overlay.rb`. |
| SV-270583 | Error messages must not reveal sensitive/harmful info (SI-11 a) | Procedural DBMS-settings/application-code review; Oracle does not expose transactional data by default, and message content is a customer application-design fact with no portable SQL predicate. Error/alert output ships to CloudWatch and Cloud.gov logs under tenant isolation (platform); content is a customer responsibility. See `controls/overlay.rb`. |
| SV-270584 | Restrict error messages to authorized personnel (SI-11 b) | Procedural review of customer end-user applications and tool access; tenant isolation of the CloudWatch/Cloud.gov log stream is a platform boundary fact, while who may view error content and error suppression in customer apps/tools are customer responsibilities with no portable SQL predicate (sibling to SV-270583). See `controls/overlay.rb`. |
| SV-270562 | Temp-password procedures for new accounts (IA-5(1)(a/d)) | Procedural, no pass/fail SQL: "If all user accounts are authenticated by the OS or an enterprise-level authentication/access mechanism, and not by Oracle, this is not a finding"; otherwise review procedures/evidence for temporary-password creation. Temporary-password issuance is an organizational process, not a queryable setting. Database credentials are issued through the FedRAMP-authorized CloudFoundry brokered-credentials model (the enterprise mechanism the check anticipates); satisfied by system documentation / the Cloud.gov SSP (IA-5). |
...
| SV-270554 | Disable unused integrated DBMS components (CM-7 a) | `allowed_oracledb_components_integrated_into_dbms` | The baseline SQL enumerates the five chopt-manageable `v$option` options (Data Mining, .NET extensions, OLAP, Partitioning, Real Application Testing) whose `value='TRUE'`. On a stock brokered RDS instance these report **FALSE**, so the enabled set is empty and the control passes with **no allowlist entry needed** — the enabled-component set is inherited from the AWS provisioning default (the tenant cannot run `chopt` on RDS). Overlay stays `inherited` (no override); baseline SQL runs in both postures. **Conditional on baseline fix** cloud-gov/oracle-database-19c-stig-baseline#14 (adds the `value='TRUE'` filter so an option reported FALSE is not false-flagged). | Any option enabled (`value='TRUE'`) and **not** authorized in the allowlist is a finding for review; the customer documents/authorizes it or requests it be disabled where possible. |

> **SV-270552 (remove default sample/demo schemas, CM-7 a)** is also inherited
> and SQL-verified but needs no allowlist: the baseline asserts the Oracle default
> sample-schema accounts (`BI`, `HR`, `OE`, `PM`, `IX`, `SH`, `SCOTT`) are absent
> from `dba_users`. A stock brokered RDS instance does not install the sample
> schemas (not part of the AWS DBCA/provisioning selection), so a compliant
...
| SV-270561 | Enforce DOD password complexity (`PASSWORD_VERIFY_FUNCTION`, IA-5(1)(a)) | Fix binds a DOD-complexity verify function to the site's profiles (`ALTER PROFILE ... LIMIT PASSWORD_VERIFY_FUNCTION <fn>`, or `ALTER USER ... PROFILE ORA_STIG_PROFILE`) — org-defined db-altering SQL; Not a Finding when accounts are OS/enterprise-authenticated rather than by Oracle. Baseline SQL presence assertion implemented upstream (mitre-baseline `feat/sv-270561-password-verify-function`, held for PR); the complexity-rule code review is satisfied by using Oracle's supplied `ORA12C_STIG_VERIFY_FUNCTION` / `ORA_STIG_PROFILE`. **Known finding on RDS:** the `RDSADMIN` profile cannot be remediated (`ALTER PROFILE RDSADMIN` is blocked, `ORA-20900`) so it can never carry a compliant `PASSWORD_VERIFY_FUNCTION` — tracked in [#77](https://github.com/cloud-gov/cg-oracle-database-19c-stig-overlay/issues/77); `DEFAULT` is customer-remediable. | run `12_password_verify_function.sql` (DEFAULT) and/or `11_ora_stig_profile.sql` (`ORA_STIG_PROFILE`) |
| SV-270563 | Password maximum lifetime (`PASSWORD_LIFE_TIME`/`PASSWORD_GRACE_TIME`, IA-5(1)(a/d)) | Fix is `ALTER PROFILE <profile> LIMIT PASSWORD_LIFE_TIME <=60 ...` (or `ALTER USER ... PROFILE ORA_STIG_PROFILE`) — org-defined db-altering SQL; Not a Finding when accounts are OS/enterprise-authenticated. Baseline check correct (`baseline_status: fixed`; effective-life-time DECODE query, `should_not cmp 'UNLIMITED'` and `<= 60` per profile). The Oracle vendor DEFAULT of `PASSWORD_LIFE_TIME 35` + `PASSWORD_GRACE_TIME 7` = 42 already satisfies the `<= 60` bound. | run `10_profiles.sql` (DEFAULT — keeps `PASSWORD_LIFE_TIME 35`, preserving the vendor `GRACE_TIME 7` warning window; 35+7=42 <= 60) |
| SV-270587 | Reject commonly-used/expected/compromised passwords (`PASSWORD_VERIFY_FUNCTION`, IA-5(1)(b)) | SV-270561 sibling: same PASSWORD_VERIFY_FUNCTION SQL predicate + remediation; the compromised-password logic lives in the verify-function code (satisfied by Oracle's `ORA12C_STIG_VERIFY_FUNCTION`, allowlisted through the `approved_password_verify_functions` input — a **function** name, not a profile name). Org-defined db-altering SQL on the SV-270495 pattern; Not a Finding when accounts are OS/enterprise-authenticated. Baseline SQL assertion merged upstream (cloud-gov/oracle-database-19c-stig-baseline#16, `cloudgov` @ `4534af38`, pinned in `inspec.lock`). **Known finding on RDS:** the `RDSADMIN` profile cannot be remediated (`ALTER PROFILE RDSADMIN` is blocked) — tracked in [#77](https://github.com/cloud-gov/cg-oracle-database-19c-stig-overlay/issues/77). | run `12_password_verify_function.sql` (DEFAULT) and/or `11_ora_stig_profile.sql` (`ORA_STIG_PROFILE`) |
...
| SV-270546 | Identify temporary/emergency accounts (CM-6 b / AC-2(2)) | DISA allows a policy forbidding temporary/emergency accounts or enterprise authentication as Not a Finding; if Oracle-managed temporary accounts are used, the customer must define, create, and assign a distinctive temporary profile. Baseline manual skip. | customer policy, or create/assign a temporary profile |
| SV-270547 | Auto-remove/disable temporary accounts after 72h (CM-6 b) | Sibling to SV-270546. DISA allows a consistently enforced policy forbidding temporary/emergency accounts, or non-Oracle (OS/enterprise) authentication, as Not a Finding. Satisfied at provisioning (broker issues a single non-temporary account); if the customer creates Oracle-managed temporary accounts, providing the 72-hour auto-lock mechanism is the customer's responsibility. Baseline manual skip (like SV-270546); not in the overlay's SQL-only skip gate. | customer policy, or run `60_temporary_users.sql` (TEMPORARY_USERS profile + 72h lock job) |
