...
| SV-270587 | Reject commonly-used/expected/compromised passwords (`PASSWORD_VERIFY_FUNCTION`, IA-5(1)(b)) | SV-270561 sibling: same PASSWORD_VERIFY_FUNCTION SQL predicate + remediation; the compromised-password logic lives in the verify-function code (satisfied by Oracle's `ORA12C_STIG_VERIFY_FUNCTION` / `ORA_STIG_PROFILE`, wired through the `approved_password_verify_functions` input). Org-defined db-altering SQL on the SV-270495 pattern; Not a Finding when accounts are OS/enterprise-authenticated. Baseline SQL presence assertion implemented upstream (mitre-baseline `feat/sv-270587-password-verify-compromised`, held for PR). | run `12_password_verify_function.sql` (DEFAULT) and/or `11_ora_stig_profile.sql` (`ORA_STIG_PROFILE`) |
