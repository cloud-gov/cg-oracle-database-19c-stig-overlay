...
| SV-270563 | Password maximum lifetime (`PASSWORD_LIFE_TIME`/`PASSWORD_GRACE_TIME`, IA-5(1)(a/d)) | Fix is `ALTER PROFILE <profile> LIMIT PASSWORD_LIFE_TIME 60 PASSWORD_GRACE_TIME 0` (or `ALTER USER ... PROFILE ORA_STIG_PROFILE`) — org-defined db-altering SQL; Not a Finding when accounts are OS/enterprise-authenticated. Baseline check correct (`baseline_status: fixed`; effective-life-time DECODE query, `should_not cmp 'UNLIMITED'` and `<= 60` per profile). | run `10_profiles.sql` (DEFAULT — sets `PASSWORD_LIFE_TIME 60` (standard DOD value) and `PASSWORD_GRACE_TIME 0`) and/or `11_ora_stig_profile.sql` (`ORA_STIG_PROFILE`) |
